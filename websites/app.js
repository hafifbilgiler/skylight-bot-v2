...
function _initScrollDetection() {
    ...
    _btn.style.cssText = `
        ...
        background: #1a1a1f; border: 1.5px solid rgba(255,255,255,0.18);
    ...
    _btn.onclick = () => {
        ...
        _btn.style.opacity = '0'; _btn.style.pointerEvents = 'none';
        _btn.style.transform = 'translateX(-50%) translateY(12px)';
    };
    const parent = chatDisplay.parentElement;
    if (parent) { parent.style.position = 'relative'; parent.appendChild(_btn); }
    chatDisplay.addEventListener('scroll', () => {
        const distFromBottom = chatDisplay.scrollHeight - chatDisplay.scrollTop - chatDisplay.clientHeight;
        _userScrolledUp = distFromBottom > 80;
        if (_userScrolledUp && chatDisplay.style.display !== 'none') {
            _btn.style.opacity = '1'; _btn.style.pointerEvents = 'auto';
...
function _scrollToBottom(force = false) {
    if (force || !_userScrolledUp) { chatDisplay.scrollTop = chatDisplay.scrollHeight; }
}
...
// ✅ FIX: Başlangıçta null — DOMContentLoaded'da konuşma listesi yüklendikten sonra set edilir
...
function setChatMode(mode) {
    ...
    if (!allowedModes.includes(mode)) {
        const modeLabels = { code: "Kod Yazıcı", it_expert: "IT Uzmanı (BETA)", student: "Öğrenci Asistanı (BETA)", social: "Sosyal Asistan (BETA)" };
        showUpgradeModal(modeLabels[mode] || mode);
        closeModeDropdown();
        return;
    }
    if (mode === "code" && !codeModelEnabled) { showToast("Kod modu henüz aktif değil", "warn"); return; }
...
function toggleModeDropdown() {
    ...
    if (menu.classList.contains('show')) { closeModeDropdown(); }
    else { menu.classList.add('show'); trigger.classList.add('open'); }
}
...
function selectMode(mode) { setChatMode(mode); }

document.addEventListener('click', (e) => { if (!e.target.closest('.mode-dropdown-wrapper')) closeModeDropdown(); });

async function checkCodeModeStatus() {
    try {
        const token = localStorage.getItem('token');
        const res = await fetch(PROXY_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "code_mode_status", token }) });
    ...
    } catch (e) { codeModelEnabled = false; }
}
...
function cleanAutofill() {
    ...
    if (val.includes('@') && !val.includes(' ') && val.length > 5) userInput.value = "";
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (urlParams.get('premium') === '1') {
        ...
        setTimeout(() => showToast('🎉 Premium üyeliğiniz aktif edildi! Hoş geldiniz.', 'info', 6000), 800);
    }
    if (urlParams.get('payment') === 'failed') {
        window.history.replaceState({}, '', window.location.pathname);
        setTimeout(() => showToast('Ödeme başarısız. Lütfen tekrar deneyin.', 'error', 5000), 500);
    }
    ...
    if (savedName && savedToken) {
        ...
        try {
            const _pRes = await fetch(PROXY_URL, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'get_profile', token: savedToken }) });
            ...
            if (_pData.user?.avatar_style) {
                ...
                if (_av) _av.src = `/avatars/${_pData.user.avatar_style}.svg`;
            }
        } catch(e) {}
        await loadConversations();

        // ✅ FIX: Konuşma listesi yüklendikten sonra ID kontrolü yap
        ...
        if (lastConvId && conversations.find(c => c.id === lastConvId)) {
            ...
            // Konuşma artık listede yok — state temizle
...
async function handleCodeRequest() {
    ...
    if (authMode === "register") {
        ...
        if (kvkk && !kvkk.checked) { kvkk.style.outline = "2px solid #ff4b4b"; return alert("Devam etmek için KVKK ve Kullanım Koşulları'nı onaylamanız gerekiyor."); }
        if (kvkk) kvkk.style.outline = "";
    }
    tempEmail = email;
    try {
        const res  = await fetch(PROXY_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "request_code", email, mode: authMode, name }) });
        const data = await res.json();
        if (res.ok && data.status === "success") {
            document.getElementById("email-step").style.display = "none";
            document.getElementById("code-step").style.display = "block";
            document.getElementById("code-sent-info").textContent = email + " adresine kod gönderildi.";
        } else { alert(data.detail || data.message || "Bir hata oluştu."); }
    } catch (e) { alert("Kod gönderilemedi."); }
}

async function verifyAndFinish() {
    const code = document.getElementById("auth-code").value.trim();
    try {
        const res  = await fetch(PROXY_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "verify_code", email: tempEmail, code, mode: authMode }) });
        const data = await res.json();
        if (res.ok && data.status === "success") { localStorage.setItem('token', data.token); localStorage.setItem('user_name', data.user.name); location.reload(); }
        else { alert(data.detail || data.message || "Doğrulama hatası."); }
    } catch (e) { alert("Doğrulama hatası."); }
}
...
function handleFileSelect(event) {
    ...
    if (imageTypes.includes(file.type)) { handleImageUpload(file); }
    else {
        if (userSubscription && !userSubscription.is_premium && !userSubscription?.features?.file_upload) { showUpgradeModal("Dosya Yükleme"); event.target.value = ""; return; }
        uploadFile(file);
    }
    event.target.value = "";
}
...
function handleImageUpload(file) {
    ...
    const MAX_DIM = 1600, QUALITY = 0.85;
    const img = new Image();
    img.onload = function() {
        ...
        if (needsResize) {
            ...
            finishImageUpload(b64, "image/jpeg", file.name || "screenshot.jpg", b64.length);
        } else {
            const reader = new FileReader();
            reader.onload = e => { const b64 = e.target.result.split(",")[1]; finishImageUpload(b64, file.type || "image/png", file.name || "image.png", file.size); };
    ...
    img.onerror = function() {
        const reader = new FileReader();
        reader.onload = e => { const b64 = e.target.result.split(",")[1]; finishImageUpload(b64, file.type || "image/png", file.name || "image.png", file.size); };
        reader.onerror = () => { removeUploadedFile(); alert("Görsel okunamadı."); };
        reader.readAsDataURL(file);
    };
...
function finishImageUpload(base64Data, mimeType, filename, displaySize) {
    ...
    const chipMeta = document.getElementById("file-chip-meta");
    const chipIcon = document.getElementById("file-chip-icon");
    uploadedImage  = { base64: base64Data, mime_type: mimeType, filename, size: displaySize };
...
async function uploadFile(file) {
    ...
    try {
        ...
        if (res.ok && data.status === "success") {
            ...
            chip.classList.remove("file-uploading");
            chipIcon.className   = `fas ${FILE_ICONS[data.file_type] || "fa-file"}`;
            chipMeta.textContent = `${FILE_TYPE_NAMES[data.file_type] || data.file_type} · ${formatFileSize(data.size_bytes)} · ${data.line_count} satır`;
            ...
            if (codeTypes.includes(data.file_type) && codeModelEnabled) { setChatMode("code"); showToast("Kod dosyası algılandı — Kod moduna geçildi", "info"); }
        } else { removeUploadedFile(); alert(data.detail || data.message || "Dosya yüklenemedi."); }
    } catch (e) { removeUploadedFile(); alert("Dosya yükleme hatası: " + e.message); }
}

function removeUploadedFile(clearActive = false) {
    uploadedFile = null;
    if (clearActive) window._activeConvFile = null;
    uploadedImage = null;
...
function renderConversations() {
    ...
    const renderGroup = (label, list) => {
        ...
        list.forEach(conv => {
            ...
            const dotColor = isActive ? '#00f2fe' : `hsl(${hue}, 60%, 62%)`;
            item.style.background = isActive ? `linear-gradient(135deg, rgba(0,242,254,0.08), rgba(0,242,254,0.04))` : '';
            item.innerHTML = `
                <div style="display:flex;align-items:center;gap:9px;">
                    <div style="width:7px;height:7px;border-radius:50%;background:${dotColor};flex-shrink:0;box-shadow:${isActive ? '0 0 6px rgba(0,242,254,0.5)' : 'none'};transition:all 0.2s;opacity:${isActive ? '1' : '0.5'};"></div>
                    <div style="flex:1;min-width:0;">
                        <div class="conv-title" style="font-size:13px;font-weight:${isActive ? '600' : '450'};color:${isActive ? '#fff' : 'rgba(255,255,255,0.72)'};letter-spacing:${isActive ? '-0.1px' : '0'};">${escapeHtml(title)}</div>
                        <div style="font-size:11px;color:rgba(255,255,255,0.28);margin-top:2px;">${timeAgo}</div>
                    </div>
                    <span class="conv-delete" onclick="deleteConversation('${conv.id}', event)" style="opacity:0;padding:4px 6px;border-radius:6px;color:rgba(255,255,255,0.25);transition:all 0.15s;flex-shrink:0;" onmouseover="this.style.color='#ff6b6b';this.style.background='rgba(255,75,75,0.1)'" onmouseout="this.style.color='rgba(255,255,255,0.25)';this.style.background=''">
...
function filterConversations(query) {
    ...
    const filtered = conversations.filter(c => (c.title || "Yeni Sohbet").toLowerCase().includes(q));
    conversationList.innerHTML = "";
    ...
    filtered.forEach(conv => {
        ...
        const title      = conv.title || "Yeni Sohbet";
        const timeAgo    = formatTimeAgo(conv.updated_at || conv.created_at);
        const highlighted = escapeHtml(title).replace(new RegExp(escapeHtml(q), 'gi'), m => `<mark style="background:rgba(0,242,254,0.25);color:var(--accent-cyan);border-radius:2px;">${m}</mark>`);
...
async function loadConversation(convId) {
    ...
    try {
        ...
        if (data.messages && data.messages.length > 0) {
            ...
            setTimeout(() => { _userScrolledUp = false; _scrollToBottom(true); const btn = document.getElementById('scroll-to-bottom-btn'); if (btn) { btn.style.opacity = '0'; btn.style.pointerEvents = 'none'; btn.style.transform = 'translateX(-50%) translateY(12px)'; } }, 100);
            renderConversations();
...
function createMessage(isUser) {
    ...
    let copyBtn = null, feedbackBar = null;
    if (!isUser) {
        const actionBar = document.createElement("div");
        actionBar.className = "msg-action-bar";
        copyBtn = document.createElement("button");
        copyBtn.className = "msg-action-btn copy-btn"; copyBtn.innerHTML = '<i class="far fa-copy"></i>'; copyBtn.title = "Kopyala";
        actionBar.appendChild(copyBtn);
        const likeBtn = document.createElement("button");
        likeBtn.className = "msg-action-btn feedback-btn like-btn"; likeBtn.innerHTML = '<i class="far fa-thumbs-up"></i>'; likeBtn.title = "Beğen";
        actionBar.appendChild(likeBtn);
        const dislikeBtn = document.createElement("button");
        dislikeBtn.className = "msg-action-btn feedback-btn dislike-btn"; dislikeBtn.innerHTML = '<i class="far fa-thumbs-down"></i>'; dislikeBtn.title = "Beğenme";
...
// ✅ FIX: renderMarkdown — TÜM bracket tag temizliği + kaynak kartı
function renderMarkdown(text) {
    if (typeof marked === 'undefined') return escapeHtml(text);

    // ── Tüm bracket tag'ları temizle ─────────────────────────
    text = text
        .replace(/\[CANLI VERİ[^\]]*\]/g, '')
        .replace(/\[\/CANLI VERİ[^\]]*\]/g, '')
        .replace(/\[WEB ARAŞTIRMA SONUÇLARI[^\]]*\]/g, '')
        .replace(/\[\/WEB ARAŞTIRMA SONUÇLARI\]/g, '')
        .replace(/\[ARAŞTIRMA SONUÇLARI[^\]]*\]/g, '')
        .replace(/\[\/ARAŞTIRMA SONUÇLARI\]/g, '')
        .replace(/\[GÜNCEL BİLGİ[^\]]*\]/g, '')
        .replace(/\[\/GÜNCEL BİLGİ\]/g, '')
        .replace(/\[BİLGİ:[^\]]*\]/g, '')
        .replace(/\[NOT:[^\]]*\]/g, '')
        .replace(/\[SADECE BU VERİYİ[^\]]*\]/g, '')
        .replace(/\[Sistem Bilgisi\]/g, '')
        .replace(/\[\/Sistem Bilgisi\]/g, '')
        .replace(/\[LIVE DATA[^\]]*\]/g, '')
        .replace(/\[Canlı Veri[^\]]*\]/g, '')
        .replace(/\[GÖREV:[^\]]*\]/g, '')
        .replace(/\[Fenerbahçe[^\]]*\]/g, '')
        .replace(/\[Galatasaray[^\]]*\]/g, '')
        .replace(/\[Beşiktaş[^\]]*\]/g, '')
        .replace(/\[Trabzonspor[^\]]*\]/g, '');

    // ── SOURCES_BLOCK_PLACEHOLDER temizliği ───────────────────
    if (text.includes('SOURCES_BLOCK_PLACEHOLDER:')) {
        text = text.slice(0, text.indexOf('SOURCES_BLOCK_PLACEHOLDER:')).trimEnd();
    }

    // ── Sources block → kaynak kartı ─────────────────────────
    const SOURCES_SEP = '\n---\n\u{1F4CE} **Kaynaklar**';
    if (text.includes(SOURCES_SEP)) {
        const sepIdx     = text.indexOf(SOURCES_SEP);
        const mainText   = text.slice(0, sepIdx);
        const sourcesRaw = text.slice(sepIdx + SOURCES_SEP.length);
        const sources    = [];
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (lm) {
                let domain = '';
                try { domain = new URL(lm[3]).hostname.replace(/^www\./, ''); } catch(e) {}
                const src = { num: lm[1], title: lm[2], url: lm[3], domain, snippet: '' };
                const sm  = (lines[i + 1] || '').match(/^\s+\*([^*]+)\*\s*·?\s*(.*)/);
                if (sm) { src.domain = sm[1].trim() || domain; src.snippet = sm[2].trim().slice(0, 90); i++; }
                sources.push(src);
            }
        }
        if (sources.length > 0) {
            const items = sources.map(s => `
                <a href="${escapeHtml(s.url)}" target="_blank" rel="noopener noreferrer" class="source-item">
                    ...
                    <div class="source-info">
                        ...
                        ${(s.domain || s.snippet) ? `<div class="source-meta">
                            ${s.domain ? `<span class="source-domain">${escapeHtml(s.domain)}</span>` : ''}
                            ${s.snippet ? ` · <span class="source-snippet">${escapeHtml(s.snippet)}</span>` : ''}
                        </div>` : ''}
                    </div>
                    <i class="fas fa-arrow-up-right-from-square source-ext-icon"></i>
                </a>`).join('');
            const sourcesHtml = '<div class="sources-card"><div class="sources-header"><i class="fas fa-link"></i><span>Kaynaklar</span><span class="sources-count">' + sources.length + '</span></div><div class="sources-list">' + items + '</div></div>';
            return (typeof marked !== 'undefined' ? marked.parse(mainText) : escapeHtml(mainText)) + sourcesHtml;
        }
    }
    // ─────────────────────────────────────────────────────────
    ...
    text = text.replace(/\[IMAGE_B64\]([\s\S]*?)\[\/IMAGE_B64\]/g, (match, b64) => {
        ...
        <div class="generated-image-card">
            <div class="generated-image-label-row">
                <span class="generated-image-badge"><i class="fas fa-sparkles"></i> ONE-BUNE Görseli</span>
            </div>
            <img src="data:image/png;base64,${b64.trim()}" alt="Oluşturulan görsel" class="generated-image-fade" loading="lazy">
            <div class="generated-image-footer">
                <div class="generated-image-tagline">AI ile oluşturuldu</div>
                <button onclick="downloadGeneratedImage(this)" class="code-copy-btn generated-image-download-btn"><i class="fas fa-download"></i> İndir</button>
            </div>
        </div>`);
        return placeholder;
    });
...
async function handleChat() {
    ...
    if (token) {
        ...
        if (userSubscription && !userSubscription.is_premium) {
            ...
            if (remaining <= 0) { showToast(`⏰ Günlük mesaj limitine ulaştın (${limit}/gün). Premium'a yükselt veya yarın tekrar dene!`, "warn"); return; }
        }
    }
    ...
    if (imageForThisMsg) {
        uploadedImage = null;
        // ✅ FIX: Chip UI'ı da temizle (önceden sadece state null'lanıyordu, chip ekranda kalıyordu)
        const chipArea = document.getElementById("file-chip-area");
        if (chipArea) chipArea.style.display = "none";
        userInput.placeholder = "Bir şeyler sor...";
    }
    if (fileForThisMsg && !window._activeConvFile) window._activeConvFile = fileForThisMsg;
    ...
    } else { userMsg.content.textContent = prompt; }
    userInput.value = "";
    ...
    // ── Client-side adım simülasyonu ──────────────────────────
    const _deepSearchKw = ['araştır','bul','son gelişme','güncel','ne oldu','neler oluyor','haber','analiz','incele','dünyada','gündem','son durum','açıkla detaylı','research','latest','find me','what happened'];
    const _liveKw   = ['dolar','euro','bitcoin','btc','hava durumu','borsa','hisse','kripto','sıcaklık','kur kaç'];
    const _codeKw   = ['çalışmıyor','hata var','error','traceback','debug','düzelt','fix','broken','crash'];
    ...
    let clientStepsContainer = null, clientStepActive = null;

    function _clientAddStep(emoji, text) {
        ...
        if (clientStepActive) {
            clientStepActive.classList.remove('active'); clientStepActive.classList.add('done');
        ...
        el.innerHTML = `<div class="step-spinner"></div><span class="step-emoji">${emoji}</span><span class="step-text">${text}</span>`;
    ...
    function _clientFinishSteps() {
        ...
        if (clientStepsContainer) {
            setTimeout(() => {
                ...
                clientStepsContainer.onclick = () => { clientStepsContainer.style.maxHeight = ''; clientStepsContainer.style.opacity = '0.8'; clientStepsContainer.onclick = null; };
            }, 4000);
        }
    }

    // Fake step animasyonları kaldırıldı.
    // Backend'de gerçek <think> reasoning gelirse zaten parser onu render eder.
    // Aksi halde dönen ONE-BUNE orb gösterilir.
    botMsg.content.innerHTML = `
        <div class="typing-indicator">
            <div class="typing-orb">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 600">
                    <defs>
                        <radialGradient id="ti-orbMain" cx="42%" cy="38%" r="62%">
                            <stop offset="0%" stop-color="#a0b8d8"/>
                            <stop offset="25%" stop-color="#5ab8d4"/>
                            <stop offset="60%" stop-color="#00c8d8"/>
                            <stop offset="100%" stop-color="#009aaa"/>
                        </radialGradient>
                        <radialGradient id="ti-highlight" cx="35%" cy="30%" r="45%">
                            <stop offset="0%" stop-color="#c0a8e0" stop-opacity="0.65"/>
                            <stop offset="60%" stop-color="#8898c8" stop-opacity="0.25"/>
                            <stop offset="100%" stop-color="#4488bb" stop-opacity="0"/>
                        </radialGradient>
                        <radialGradient id="ti-rimLight" cx="50%" cy="85%" r="40%">
                            <stop offset="0%" stop-color="#00e8f8" stop-opacity="0.4"/>
                            <stop offset="100%" stop-color="#00c8d8" stop-opacity="0"/>
                        </radialGradient>
                    </defs>
                    <circle cx="300" cy="300" r="213" fill="none" stroke="#00d8e8" stroke-width="1.5" opacity="0.25"/>
                    <circle cx="300" cy="300" r="210" fill="url(#ti-orbMain)"/>
                    <circle cx="300" cy="300" r="210" fill="url(#ti-highlight)"/>
                    <circle cx="300" cy="300" r="210" fill="url(#ti-rimLight)"/>
                    <ellipse cx="265" cy="205" rx="95" ry="52" fill="white" opacity="0.10" transform="rotate(-15,265,205)"/>
                </svg>
            </div>
            <span class="typing-text">Düşünüyor...</span>
        </div>`;

    const requestBody = { action: "chat", prompt, mode: "assistant", conversation_id: currentConversationId, token: localStorage.getItem('token'), thinking_mode: window._thinkingMode || "none" };
    ...
    try {
        ...
        let fullText = "", stepsContainer = null, _stepActive = null;
        let clientStepsDone = false;

        function _doneClientSteps() {
            if (!clientStepsDone) {
                clientStepsDone = true;
                // Fake step kaldırıldı, sadece typing indicator'ı sil
        ...
        function _addStep(line) {
            ...
            const emoji = match?.[1] || '⚙️', text = match?.[2] || inner;
            if (!stepsContainer) {
            ...
            if (_stepActive) {
                _stepActive.classList.remove('active'); _stepActive.classList.add('done');
        ...
        function _finishSteps() {
            ...
            if (stepsContainer) {
                setTimeout(() => {
                    ...
                    stepsContainer.onclick = () => { stepsContainer.style.maxHeight = ''; stepsContainer.style.opacity = '0.7'; stepsContainer.style.cursor = ''; stepsContainer.onclick = null; };
                }, 5000);
            }
        }
        ...
        <div class="onebune-image-loading">
            <div class="onebune-image-loading-wrap">
                <div class="onebune-loader-orb"><div class="onebune-loader-core"><div class="onebune-loader-text">ONE-BUNE</div></div></div>
        ...
        while (true) {
            ...
            let remainingChunk = '', buf = window._stepBuffer;
            ...
            } else { remainingChunk += unprocessed; window._stepBuffer = ''; }
            if (remainingChunk.includes('[STEPS_DONE]')) { remainingChunk = remainingChunk.replace('[STEPS_DONE]', ''); _finishSteps(); }

            // ✅ FIX: Tüm bracket tag'ları stream'den temizle
            ...
            // ✅ THINKING MODE — reasoning content stream parser
            // Backend reasoning'i <think>...</think> ile sarıyor
            if (!window._thinkBuf) window._thinkBuf = '';
            window._thinkBuf += remainingChunk;
            let outsideThink = '';
            let thinkActive = window._thinkActive || false;
            let buf2 = window._thinkBuf;
            window._thinkBuf = '';
            let i = 0;
            while (i < buf2.length) {
                if (!thinkActive) {
                    const o = buf2.indexOf('<think>', i);
                    if (o === -1) { outsideThink += buf2.slice(i); break; }
                    outsideThink += buf2.slice(i, o);
                    thinkActive = true;
                    _thinkOpen(botMsg);
                    i = o + 7;
                } else {
                    const c = buf2.indexOf('</think>', i);
                    if (c === -1) {
                        // partial — kalan reasoning chunk
                        const rest = buf2.slice(i);
                        // Tag bölünmüş olabilir (ör. "</thi"), sondaki potansiyel parça bekle
                        const lastLT = rest.lastIndexOf('<');
                        if (lastLT >= 0 && rest.slice(lastLT).startsWith('<') && rest.length - lastLT < 8) {
                            _thinkAppend(rest.slice(0, lastLT));
                            window._thinkBuf = rest.slice(lastLT);
                        } else {
                            _thinkAppend(rest);
                        }
                        break;
                    }
                    _thinkAppend(buf2.slice(i, c));
                    _thinkClose();
                    thinkActive = false;
                    i = c + 8;
                }
            }
            window._thinkActive = thinkActive;
            remainingChunk = outsideThink;

            if (remainingChunk.includes('SOURCES_BLOCK_PLACEHOLDER:')) {
                remainingChunk = remainingChunk.slice(0, remainingChunk.indexOf('SOURCES_BLOCK_PLACEHOLDER:')).trimEnd();
            }

            fullText += remainingChunk;
            if (firstChunk && fullText.trim()) { firstChunk = false; _doneClientSteps(); }

            let renderText = fullText;
            const hasOpenTag  = renderText.includes("[IMAGE_B64]");
            const hasCloseTag = renderText.includes("[/IMAGE_B64]");
            const isImageGenerating = fullText.includes("🎨 Görsel Oluşturuluyor") || fullText.includes("🎨 Görsel oluşturuluyor") || fullText.includes("Bu işlem 30-60 saniye sürebilir");
            renderText = renderText.replace("🎨 Görsel Oluşturuluyor...", "").replace("🎨 Görsel oluşturuluyor...", "").replace("🖼️ Görsel yükleniyor...", "").replace("Görsel Oluşturuluyor...", "").replace("Görsel oluşturuluyor...", "").replace("⏳ Bu işlem 30-60 saniye sürebilir...", "").replace("⏳ Bu işlem 30–60 saniye sürebilir...", "").replace("Bu işlem 30-60 saniye sürebilir...", "").trim();
            if (hasOpenTag && !hasCloseTag) { const idx = renderText.indexOf("[IMAGE_B64]"); renderText = renderText.substring(0, idx) + IMAGE_LOADING_PLACEHOLDER; }
            if (!hasOpenTag && isImageGenerating) { renderText = IMAGE_LOADING_PLACEHOLDER; }

            // ✅ FIX: [STEP] chunk gelince renderText boş olur — stepsContainer'ı silme
            if (renderText.trim() || renderText.includes("onebune-image-loading")) {
        ...
        // ✅ FIX: final temizlik — tüm bracket tag'ları
        const cleanedFinalText = fullText
            ...
            .replace(/\[\/CANLI VERİ[^\]]*\]/g, '')
            .replace(/\[Canlı Veri[^\]]*\]/g, '')
            .replace(/SOURCES_BLOCK_PLACEHOLDER:[\s\S]*/g, '')
            .trim();

        const typingEl = botMsg.content.querySelector('.typing-indicator');
        if (typingEl) typingEl.remove();
        window._stepBuffer = '';
        if (fullText.trim()) botMsg.content.innerHTML = renderMarkdown(cleanedFinalText);
        else if (firstChunk) botMsg.content.innerHTML = "";

        if (botMsg.copyBtn) {
            botMsg.copyBtn.onclick = () => {
                const cleanText = fullText
                    .replace("🎨 Görsel oluşturuluyor...", "").replace("🖼️ Görsel yükleniyor...", "").replace("Görsel oluşturuluyor...", "")
                    .replace(/\[IMAGE_B64\][\s\S]*?\[\/IMAGE_B64\]/g, '[Oluşturulan görsel]').trim();
        ...
        if (token) await loadSubscriptionStatus();

    } catch (e) {
        if (e.name === 'AbortError') {
            if (botMsg.content.textContent === '' || botMsg.content.querySelector('.typing-indicator')) botMsg.content.innerHTML = '<span style="color:var(--text-muted);font-style:italic;">⏹ Yanıt durduruldu.</span>';
        } else { botMsg.content.innerHTML = '<span style="color:var(--danger-color);">Bağlantı hatası. Lütfen tekrar deneyin.</span>'; }
    } finally { isSending = false; currentAbortController = null; showSendButton(); userInput.focus(); }
}
...
userInput.addEventListener("paste", (e) => {
    ...
    if (items) {
        for (let i = 0; i < items.length; i++) {
            if (items[i].type.startsWith("image/")) { e.preventDefault(); const file = items[i].getAsFile(); if (file) { handleImageUpload(file); showToast("Görsel yapıştırıldı", "info"); } return; }
        }
    }
    const text = clipboardData.getData("text");
    if (text && text.length > 300) {
        e.preventDefault();
        if (text.length > 50000) { showToast("İçerik çok büyük — maksimum 50.000 karakter yapıştırabilirsiniz", "warn"); return; }
        pastedTextContent = text; _showPasteChip(text); return;
    }
...
function _showPasteChip(text) {
    ...
    const lines = text.split('\n').length, words = text.trim().split(/\s+/).length;
    ...
    const icon    = isCode ? 'fa-code' : 'fa-align-left', label = isCode ? 'Kod' : 'Metin';
    const chipArea = document.getElementById('file-chip-area');
    chipArea.style.display = 'block';
    if (!uploadedFile && !uploadedImage) { const existingFileChip = document.getElementById('file-chip'); if (existingFileChip) existingFileChip.style.display = 'none'; }
    const chip = document.createElement('div');
    chip.id = 'paste-chip';
    chip.style.cssText = `display:flex;align-items:flex-start;gap:10px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:10px 12px;position:relative;max-width:280px;margin-bottom:6px;cursor:default;`;
    chip.innerHTML = `
        <div style="width:34px;height:34px;min-width:34px;border-radius:8px;background:linear-gradient(135deg,rgba(0,242,254,.12),rgba(188,78,253,.1));display:flex;align-items:center;justify-content:center;"><i class="fas ${icon}" style="font-size:13px;color:var(--accent-cyan);"></i></div>
        <div style="flex:1;min-width:0;">
            <div style="font-size:12px;font-weight:600;color:#eeeef0;margin-bottom:2px;">Yapıştırılan ${label}</div>
            <div style="font-size:10px;color:#55556a;">${lines} satır · ${words} kelime</div>
            <div style="font-size:10px;color:#8e8ea0;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:180px;">${preview}</div>
        </div>
        <button onclick="_removePasteChip()" style="position:absolute;top:6px;right:6px;width:18px;height:18px;border-radius:50%;background:rgba(255,255,255,.08);border:none;color:#55556a;cursor:pointer;font-size:10px;display:flex;align-items:center;justify-content:center;"><i class="fas fa-xmark"></i></button>
    `;
...
function _removePasteChip() {
    ...
    const chipArea = document.getElementById('file-chip-area');
    if (chipArea && !uploadedFile && !uploadedImage) chipArea.style.display = 'none';
}
...
function isAtTop() { return chatDisplay.style.display === "none" || (chatDisplay.scrollTop <= 2 && !_userScrolledUp); }
window.addEventListener("touchstart", (e) => { if (document.activeElement === userInput) return; if (isAtTop()) { ptrStartY = e.touches[0].clientY; ptrActive = true; ptrTriggered = false; } else { ptrActive = false; } }, { passive: true });
window.addEventListener("touchmove", (e) => { if (!ptrActive) return; const diff = e.touches[0].clientY - ptrStartY; if (diff > 0 && diff < 160) { ptr.style.transform = `translateY(${Math.min(diff, 80)}px)`; ptrTriggered = diff > PTR_THRESHOLD; ptr.classList.toggle("active", ptrTriggered); } }, { passive: true });
window.addEventListener("touchend", () => { if (!ptrActive) return; if (ptrTriggered) location.reload(); else { ptr.style.transform = "translateY(0)"; ptr.classList.remove("active"); } ptrActive = false; });
...
async function loadSubscriptionStatus() {
    const token = localStorage.getItem('token');
    if (!token) { userSubscription = null; updateUIForSubscription(); return; }
    try {
        const res  = await fetch(PROXY_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "subscription_status", token }) });
        const data = await res.json();
        if (data.plan_id) { userSubscription = data; localStorage.setItem('user_plan', data.plan_id); }
        else { userSubscription = { plan_id: "free", is_premium: false, features: { allowed_modes: ["assistant"] }, limits: { daily_messages: 50 }, usage_today: { messages_sent: 0, messages_remaining: 50, daily_message_limit: 50 } }; }
    } catch (e) {
        userSubscription = { plan_id: "free", is_premium: false, features: { allowed_modes: ["assistant"] }, limits: { daily_messages: 50 }, usage_today: { messages_sent: 0, messages_remaining: 50, daily_message_limit: 50 } };
    }
    updateUIForSubscription();
}

function updateUIForSubscription() { updateModeDropdownLocks(); updateSidebarPlanBadge(); updateUsageIndicator(); updateAttachButton(); }

function updateModeDropdownLocks() {
    const allowedModes = userSubscription?.features?.allowed_modes || ["assistant"];
    document.querySelectorAll('.mode-dropdown-item').forEach(item => {
        const mode = item.dataset.mode, isAllowed = allowedModes.includes(mode);
        const lockIcon = item.querySelector('.mode-lock-icon'), checkIcon = item.querySelector('.item-check');
        if (!isAllowed) {
            item.classList.add('mode-locked');
            if (!lockIcon) { const lock = document.createElement('span'); lock.className = 'mode-lock-icon'; lock.innerHTML = '<i class="fas fa-lock"></i>'; if (checkIcon) checkIcon.style.display = 'none'; item.appendChild(lock); }
        } else { item.classList.remove('mode-locked'); if (lockIcon) lockIcon.remove(); if (checkIcon) checkIcon.style.display = ''; }
    });
}
...
function updateSidebarPlanBadge() {
    ...
    if (!badge) {
        badge = document.createElement('div'); badge.id = 'plan-badge-container';
    ...
    if (!userSubscription || !localStorage.getItem('token')) { badge.style.display = 'none'; return; }
    badge.style.display = 'block';
    if (userSubscription.is_premium) {
        badge.innerHTML = `<div class="plan-card premium-card" onclick="showSubscriptionPanel()"><div class="plan-card-header"><div class="plan-badge-compact premium"><i class="fas fa-crown"></i><span>Premium</span></div><i class="fas fa-chevron-right plan-card-arrow"></i></div><div class="plan-card-subtitle">Tüm özellikler aktif</div></div>`;
    } else {
        const usage = userSubscription.usage_today || {}, sent = usage.messages_sent || 0, limit = usage.daily_message_limit || 50, remaining = usage.messages_remaining || (limit - sent), pct = Math.min(100, Math.round((sent / limit) * 100));
        badge.innerHTML = `<div class="plan-card free-card" onclick="showUpgradeModal()"><div class="plan-card-header"><div class="plan-badge-compact free"><i class="fas fa-sparkles"></i><span>Free</span></div><button class="plan-upgrade-compact" onclick="event.stopPropagation(); showUpgradeModal();"><i class="fas fa-arrow-up"></i> Upgrade</button></div><div class="usage-stats"><div class="usage-main"><span class="usage-count">${remaining}</span><span class="usage-label">messages left today</span></div><div class="usage-total">${sent} of ${limit} used</div></div><div class="usage-progress-bar"><div class="usage-progress-fill" style="width:${pct}%"></div></div></div>`;
    }
}
...
function initSidebarFooter() {
    ...
    footer.innerHTML = `
        ...
        .footer-trigger { width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 9px 12px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-radius: 10px; cursor: pointer; transition: all 0.2s; color: var(--text-secondary); font-size: 12px; font-weight: 600; letter-spacing: 0.3px; margin-bottom: 0; }
        .footer-trigger:hover { background: rgba(255,255,255,0.06); border-color: rgba(255,255,255,0.12); color: var(--text-primary); }
        .footer-trigger-left { display: flex; align-items: center; gap: 8px; }
        .footer-trigger-left i { color: var(--accent-cyan); font-size: 11px; }
        .footer-chevron { font-size: 10px; color: var(--text-muted); transition: transform 0.25s; }
        .footer-accordion-item.open .footer-chevron { transform: rotate(180deg); }
        .footer-accordion-item.open .footer-trigger { border-radius: 10px 10px 0 0; border-bottom-color: transparent; background: rgba(255,255,255,0.05); }
        .footer-panel { max-height: 0; overflow: hidden; transition: max-height 0.3s ease; background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.07); border-top: none; border-radius: 0 0 10px 10px; }
        .footer-accordion-item.open .footer-panel { max-height: 400px; }
        .footer-panel-inner { padding: 12px 10px 10px; }
        .footer-links-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 3px; margin-bottom: 10px; }
        .footer-lnk { display: flex; align-items: center; gap: 7px; padding: 7px 10px; border-radius: 7px; font-size: 12px; font-weight: 500; color: var(--text-secondary); text-decoration: none; transition: all 0.15s; white-space: nowrap; }
        .footer-lnk:hover { background: rgba(0,242,254,0.07); color: var(--accent-cyan); }
        .footer-lnk i { font-size: 11px; width: 14px; text-align: center; color: var(--text-muted); flex-shrink: 0; }
        .footer-lnk:hover i { color: var(--accent-cyan); }
        .footer-divider { height: 1px; background: rgba(255,255,255,0.05); margin: 8px 0; }
        .footer-payment { display: flex; align-items: center; justify-content: center; gap: 12px; padding: 8px 0; }
        .footer-payment-logo { height: 22px; opacity: 0.85; filter: brightness(1.1); transition: opacity 0.2s, transform 0.2s; object-fit: contain; }
        .footer-payment-logo:hover { opacity: 1; transform: scale(1.05); }
        .footer-brand { text-align: center; padding-top: 8px; }
        .footer-brand-name { font-size: 11px; font-weight: 700; color: var(--text-secondary); letter-spacing: 0.5px; }
        .footer-brand-sub { font-size: 9px; color: var(--text-muted); margin-top: 2px; letter-spacing: 0.3px; }
        </style>
        <div class="footer-accordion">
            <div class="footer-accordion-item" id="footer-acc">
                <button class="footer-trigger" onclick="toggleFooterAccordion(this)">
                    <span class="footer-trigger-left"><i class="fas fa-ellipsis"></i>Daha Fazla / More</span>
                    <i class="fas fa-chevron-down footer-chevron"></i>
                </button>
                <div class="footer-panel">
                    <div class="footer-panel-inner">
                        <div class="footer-links-grid">
                            <a href="/legal/hizmet-sartlari.php" class="footer-lnk" target="_blank"><i class="far fa-file-lines"></i> Hizmet Şartları</a>
                            <a href="/legal/gizlilik-politikasi.php" class="footer-lnk" target="_blank"><i class="fas fa-shield-halved"></i> Gizlilik</a>
                            <a href="/legal/cerez-politikasi.php" class="footer-lnk" target="_blank"><i class="fas fa-cookie-bite"></i> Çerezler</a>
                            <a href="/legal/kvkk-aydinlatma.php" class="footer-lnk" target="_blank"><i class="fas fa-balance-scale"></i> KVKK</a>
                            <a href="/legal/abonelik-kosullari.php" class="footer-lnk" target="_blank"><i class="fas fa-crown"></i> Abonelik</a>
                            <a href="/legal/iade-iptal.php" class="footer-lnk" target="_blank"><i class="fas fa-rotate-left"></i> İade & İptal</a>
                            <a href="/legal/mesafeli-satis.php" class="footer-lnk" target="_blank"><i class="fas fa-file-contract"></i> Mesafeli Satış</a>
                            <a href="/legal/sirket-bilgileri.php" class="footer-lnk" target="_blank"><i class="fas fa-building"></i> Şirket</a>
                            <button onclick="showSupportModal()" class="footer-lnk" style="background:none;border:none;cursor:pointer;font-family:inherit;"><i class="fas fa-headset"></i> Destek</button>
                        </div>
                        ...
                        <div class="footer-divider"></div>
                        <div class="footer-brand"><div class="footer-brand-name">ONE-BUNE AI</div><div class="footer-brand-sub">SKYMERGE TECHNOLOGY · © 2026</div></div>
                    </div>
                </div>
            </div>
        </div>
...
function toggleFooterAccordion(trigger) {
    const item = trigger.closest('.footer-accordion-item'), isOpen = item.classList.contains('open');
...
function updateUsageIndicator() {
    ...
    if (!userSubscription || userSubscription.is_premium) { if (indicator) indicator.style.display = 'none'; return; }
    const usage = userSubscription.usage_today || {}, remaining = usage.messages_remaining;
    if (remaining !== undefined && remaining >= 0 && remaining <= 10) {
        if (!indicator) { indicator = document.createElement('div'); indicator.id = 'usage-indicator'; const mc = document.querySelector('.mode-selector-container'); if (mc) mc.parentNode.insertBefore(indicator, mc); }
...
function updateAttachButton() {
    ...
    if (!canUpload && userSubscription?.plan_id === 'free') {
        btn.classList.add('feature-locked'); btn.title = 'Dosya yükleme — Premium özellik';
        btn.onclick = (e) => { e.preventDefault(); showUpgradeModal("Dosya Yükleme"); };
    } else { btn.classList.remove('feature-locked'); btn.title = 'Dosya veya Görsel Yükle'; btn.onclick = () => document.getElementById('file-input').click(); }
}
...
function showUpgradeModal(lockedFeature) {
    ...
    modal.id = 'upgrade-modal'; modal.className = 'upgrade-modal-overlay';
    ...
    modal.innerHTML = `
        <div class="upgrade-modal-content">
            ...
            <div class="upgrade-modal-header">
                ...
                <h2>Premium'a Yükselt</h2><p>Tüm özelliklerin kilidini aç</p>
            </div>
            ...
            <div class="upgrade-single-price">
                <div class="upgrade-price-big"><span class="price-currency">₺</span><span class="price-number">64</span><span class="price-period">/ay</span></div>
            ...
            <div style="margin-top:14px;display:flex;flex-direction:column;gap:8px;">
                <div style="font-size:11px;color:var(--text-muted);margin-bottom:2px;"><i class="fas fa-file-invoice"></i> Fatura Bilgileri</div>
                <input id="checkout-phone" type="tel" placeholder="Telefon: 05XX XXX XX XX" maxlength="13" style="width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:10px 14px;color:#fff;font-size:13px;font-family:inherit;outline:none;box-sizing:border-box;" onfocus="this.style.borderColor='rgba(0,242,254,0.4)'" onblur="this.style.borderColor='rgba(255,255,255,0.1)'"/>
                <input id="checkout-identity" type="text" placeholder="TC Kimlik No (11 hane)" maxlength="11" style="width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:10px 14px;color:#fff;font-size:13px;font-family:inherit;outline:none;box-sizing:border-box;" onfocus="this.style.borderColor='rgba(0,242,254,0.4)'" onblur="this.style.borderColor='rgba(255,255,255,0.1)'"/>
                <input id="checkout-address" type="text" placeholder="Açık adres" style="width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:10px 14px;color:#fff;font-size:13px;font-family:inherit;outline:none;box-sizing:border-box;" onfocus="this.style.borderColor='rgba(0,242,254,0.4)'" onblur="this.style.borderColor='rgba(255,255,255,0.1)'"/>
                <div style="display:flex;gap:8px;">
                    <input id="checkout-city" type="text" placeholder="Şehir" style="flex:1;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:10px 14px;color:#fff;font-size:13px;font-family:inherit;outline:none;box-sizing:border-box;" onfocus="this.style.borderColor='rgba(0,242,254,0.4)'" onblur="this.style.borderColor='rgba(255,255,255,0.1)'"/>
                    <input id="checkout-zip" type="text" placeholder="Posta Kodu" maxlength="5" style="width:120px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:10px 14px;color:#fff;font-size:13px;font-family:inherit;outline:none;box-sizing:border-box;" onfocus="this.style.borderColor='rgba(0,242,254,0.4)'" onblur="this.style.borderColor='rgba(255,255,255,0.1)'"/>
                </div>
            </div>
            <div style="display:flex;flex-direction:column;gap:8px;margin-top:14px;padding:12px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:10px;">
                <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;font-size:12px;color:var(--text-secondary);line-height:1.5;"><input type="checkbox" id="chk-hizmet" style="margin-top:2px;accent-color:#00f2fe;flex-shrink:0;width:15px;height:15px;cursor:pointer;"><span><a href="/legal/hizmet-sartlari.php" target="_blank" style="color:var(--accent-cyan);text-decoration:none;" onclick="event.stopPropagation()">Hizmet Şartları</a>'nı okudum ve kabul ediyorum. <span style="color:#ff4b4b;">*</span></span></label>
                <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;font-size:12px;color:var(--text-secondary);line-height:1.5;"><input type="checkbox" id="chk-gizlilik" style="margin-top:2px;accent-color:#00f2fe;flex-shrink:0;width:15px;height:15px;cursor:pointer;"><span><a href="/legal/gizlilik-politikasi.php" target="_blank" style="color:var(--accent-cyan);text-decoration:none;" onclick="event.stopPropagation()">Gizlilik Politikası</a>'nı ve <a href="/legal/kvkk-aydinlatma.php" target="_blank" style="color:var(--accent-cyan);text-decoration:none;" onclick="event.stopPropagation()">KVKK Aydınlatma Metni</a>'ni okudum ve onaylıyorum. <span style="color:#ff4b4b;">*</span></span></label>
                <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;font-size:12px;color:var(--text-secondary);line-height:1.5;"><input type="checkbox" id="chk-abonelik" style="margin-top:2px;accent-color:#00f2fe;flex-shrink:0;width:15px;height:15px;cursor:pointer;"><span><a href="/legal/abonelik-kosullari.php" target="_blank" style="color:var(--accent-cyan);text-decoration:none;" onclick="event.stopPropagation()">Abonelik Koşulları</a>'nı ve <a href="/legal/mesafeli-satis.php" target="_blank" style="color:var(--accent-cyan);text-decoration:none;" onclick="event.stopPropagation()">Mesafeli Satış Sözleşmesi</a>'ni okudum ve kabul ediyorum. <span style="color:#ff4b4b;">*</span></span></label>
                <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;font-size:12px;color:var(--text-secondary);line-height:1.5;"><input type="checkbox" id="chk-iletisim" style="margin-top:2px;accent-color:#00f2fe;flex-shrink:0;width:15px;height:15px;cursor:pointer;"><span>Kampanya, indirim ve yenilikler hakkında e-posta / SMS ile bilgilendirme almak istiyorum. <span style="color:var(--text-muted);font-size:11px;">(isteğe bağlı)</span></span></label>
            </div>
...
function closeUpgradeModal() { const modal = document.getElementById('upgrade-modal'); if (modal) { modal.classList.remove('show'); setTimeout(() => modal.remove(), 300); } }
...
function showSubscriptionPanel() {
    ...
    panel.id = 'subscription-panel'; panel.className = 'upgrade-modal-overlay';
...
function showCheckoutModal(formContent) {
    ...
    modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.88);backdrop-filter:blur(10px);z-index:99999;display:flex;align-items:center;justify-content:center;padding:16px;box-sizing:border-box;';
    modal.onclick = (e) => { if (e.target === modal) modal.remove(); };
    const box = document.createElement('div');
    box.style.cssText = 'background:#fff;border-radius:20px;width:100%;max-width:560px;max-height:92vh;overflow-y:auto;position:relative;box-shadow:0 24px 80px rgba(0,0,0,0.6);-webkit-overflow-scrolling:touch;';
    const closeBtn = document.createElement('button');
    closeBtn.innerHTML = '✕';
    closeBtn.style.cssText = 'position:sticky;top:0;float:right;margin:10px 10px 0 0;background:rgba(0,0,0,0.07);border:none;width:32px;height:32px;border-radius:50%;cursor:pointer;font-size:14px;z-index:10;';
    closeBtn.onclick = () => modal.remove();
    const formDiv = document.createElement('div');
    formDiv.style.cssText = 'padding:0;min-height:400px;';
    formDiv.innerHTML = formContent;
    box.appendChild(closeBtn); box.appendChild(formDiv); modal.appendChild(box); document.body.appendChild(modal);
    formDiv.querySelectorAll('script').forEach(s => { const ns = document.createElement('script'); if (s.src) ns.src = s.src; else ns.textContent = s.textContent; document.body.appendChild(ns); });
    const obs = new MutationObserver(() => { const iframe = box.querySelector('iframe'); if (iframe) { iframe.style.width = '100%'; iframe.style.minHeight = '520px'; iframe.style.border = 'none'; iframe.style.display = 'block'; obs.disconnect(); } });
...
async function handleUpgrade() {
    const token = localStorage.getItem('token');
    if (!token) { showToast("Lütfen giriş yapın", "warn"); return; }
    if (userSubscription && userSubscription.is_premium) { showToast("Zaten Premium üyesiniz!", "info"); return; }
    const chkHizmet = document.getElementById('chk-hizmet'), chkGizlilik = document.getElementById('chk-gizlilik'), chkAbonelik = document.getElementById('chk-abonelik');
    if (!chkHizmet?.checked)   { showToast("Hizmet Şartları'nı kabul etmeniz gerekiyor", "warn");   return; }
    if (!chkGizlilik?.checked) { showToast("Gizlilik Politikası'nı onaylamanız gerekiyor", "warn"); return; }
    if (!chkAbonelik?.checked) { showToast("Abonelik Koşulları'nı kabul etmeniz gerekiyor", "warn"); return; }
    const phone = document.getElementById('checkout-phone')?.value.trim() || '', identity = document.getElementById('checkout-identity')?.value.trim() || '', address = document.getElementById('checkout-address')?.value.trim() || '', city = document.getElementById('checkout-city')?.value.trim() || '', zip = document.getElementById('checkout-zip')?.value.trim() || '';
    const highlight = (el) => { if(el) { el.style.borderColor='rgba(255,75,75,0.5)'; el.focus(); } };
    if (!phone || phone.replace(/\D/g,'').length < 10) { showToast("Geçerli bir telefon numarası girin", "warn"); highlight(document.getElementById('checkout-phone')); return; }
    if (!identity || identity.replace(/\D/g,'').length !== 11) { showToast("TC Kimlik No 11 hane olmalı", "warn"); highlight(document.getElementById('checkout-identity')); return; }
    if (!address) { showToast("Adres girin", "warn"); highlight(document.getElementById('checkout-address')); return; }
    if (!city)    { showToast("Şehir girin", "warn"); highlight(document.getElementById('checkout-city')); return; }
    const btn = document.getElementById('upgrade-cta');
    if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Ödeme sayfası yükleniyor...'; }
    try {
        const res = await fetch(PROXY_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'payment_checkout', token, gsmNumber: phone, identityNumber: identity, address, city, zipCode: zip || '34000' }) });
        const data = await res.json();
        if (!res.ok) throw new Error(data.detail || data.message || 'Checkout başlatılamadı');
        if (data.checkoutFormContent) showCheckoutModal(data.checkoutFormContent);
        else if (data.paymentPageUrl) window.location.href = data.paymentPageUrl;
        else throw new Error('Checkout formu alınamadı');
    } catch (e) {
        showToast("Ödeme başlatılamadı: " + e.message, "error");
        if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-crown"></i> Premium\'a Geç — ₺64/ay'; }
    }
}

async function handlePremiumCheckout() {
    const token = localStorage.getItem('token');
    if (!token) { showToast('Lütfen giriş yapın', 'error'); return; }
    const status = await checkSubscriptionStatus();
    if (status && status.subscription_active) { showToast('Zaten premium üyesiniz!', 'info'); return; }
    try {
        const checkoutBtn = document.getElementById('premium-checkout-btn');
        if (checkoutBtn) { checkoutBtn.disabled = true; checkoutBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Hazırlanıyor...'; }
        const response = await fetch(PROXY_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'payment_checkout', token }) });
        const data = await response.json();
        if (!response.ok) throw new Error(data.detail || data.message || 'Checkout başlatılamadı');
        if (data.checkoutFormContent) showCheckoutModal(data.checkoutFormContent);
        else throw new Error('Checkout formu alınamadı');
    } catch (error) {
        showToast('Ödeme başlatılamadı: ' + error.message, 'error');
        const checkoutBtn = document.getElementById('premium-checkout-btn');
        if (checkoutBtn) { checkoutBtn.disabled = false; checkoutBtn.innerHTML = '<i class="fas fa-crown"></i> Premium\'a Geç'; }
    }
}
...
async function checkSubscriptionStatus() {
    ...
    try {
        const response = await fetch(PROXY_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'subscription_status', token }) });
...
async function showSettingsModal() {
    ...
    try {
        ...
        profile = (await pRes.json()) || {}; usage = (await uRes.json()) || {}; history = (await hRes.json()) || [];
    } catch(e) {}
    const user = profile.user || {}, sub = userSubscription || {}, isPremium = sub.is_premium || usage.is_premium || false, isCancelled = sub.cancel_at_period_end || false;
    const periodEnd = sub.current_period_end ? new Date(sub.current_period_end).toLocaleDateString('tr-TR', {day:'numeric', month:'long', year:'numeric'}) : (sub.period_end || '');
    const todayUsed = usage.today_messages || 0, dailyLimit = usage.daily_limit || 50, usagePct = Math.min(100, Math.round((todayUsed / dailyLimit) * 100));
    const avatarStyle = user.avatar_style || localStorage.getItem('avatar_style') || 'av-01';
    const DICEBEAR_STYLES = [{id:'av-01',label:'Asistan'},{id:'av-02',label:'Yazılımcı'},{id:'av-03',label:'Tasarımcı'},{id:'av-04',label:'Mühendis'},{id:'av-05',label:'Robotik'},{id:'av-06',label:'Mor'},{id:'av-07',label:'Neon'},{id:'av-08',label:'Turuncu'},{id:'av-09',label:'Siborg'},{id:'av-10',label:'Lavanta'},{id:'av-11',label:'Lacivert'},{id:'av-12',label:'Yeşil'},{id:'av-13',label:'Mavi-Mor'},{id:'av-14',label:'Altın'},{id:'av-15',label:'Zümrüt'},{id:'av-16',label:'Pembe'},{id:'av-17',label:'Çelik'},{id:'av-18',label:'Kahve'},{id:'av-19',label:'Gök'},{id:'av-20',label:'Violet'},{id:'av-21',label:'Haki'},{id:'av-22',label:'Derin Mor'},{id:'av-23',label:'Nane'},{id:'av-24',label:'Kırmızı'},{id:'av-25',label:'Lacivert2'},{id:'av-26',label:'Bal'},{id:'av-27',label:'Deniz'},{id:'av-28',label:'Gümüş'},{id:'av-29',label:'Taba'},{id:'av-30',label:'Körfez'},{id:'av-31',label:'Orkide'},{id:'av-32',label:'Fıstık'},{id:'av-33',label:'Ametist'},{id:'av-34',label:'Bronz'},{id:'av-35',label:'Safir'},{id:'av-36',label:'Kızıl'},{id:'av-37',label:'Gece'},{id:'av-38',label:'Zeytin'},{id:'av-39',label:'Erik'},{id:'av-40',label:'Denim'},{id:'av-41',label:'Antrasit'},{id:'av-42',label:'Ruby'},{id:'av-43',label:'Orman'},{id:'av-44',label:'Altın2'},{id:'av-45',label:'İndigo'},{id:'av-46',label:'Gül'},{id:'av-47',label:'Arktik'},{id:'av-48',label:'Üzüm'},{id:'av-49',label:'Ceviz'},{id:'av-50',label:'Yeşim'}];
    const modal = document.createElement('div');
...
.sm-tab:hover{color:#8e8ea0;} .sm-tab.active{color:var(--accent-cyan);border-bottom-color:var(--accent-cyan);}
.sm-body{padding:18px 22px 22px;} .sm-section{display:none;} .sm-section.active{display:block;}
.sm-field{margin-bottom:14px;} .sm-lbl{font-size:10px;font-weight:700;color:#55556a;text-transform:uppercase;letter-spacing:.6px;margin-bottom:5px;display:block;}
...
.sm-primary{background:linear-gradient(135deg,#00f2fe,#bc4efd);color:#0a0a0c;} .sm-primary:hover{opacity:.9;transform:translateY(-1px);}
.sm-ghost{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);color:#8e8ea0;} .sm-ghost:hover{background:rgba(255,255,255,.08);color:#fff;}
.sm-danger{background:rgba(255,75,75,.08);border:1px solid rgba(255,75,75,.18);color:#ff4b4b;} .sm-danger:hover{background:rgba(255,75,75,.15);}
...
.sm-row-lbl{font-size:13px;color:#eeeef0;font-weight:500;} .sm-row-sub{font-size:11px;color:#55556a;margin-top:2px;}
...
.sm-stat-v{font-size:20px;font-weight:800;color:#fff;} .sm-stat-l{font-size:10px;color:#55556a;margin-top:2px;}
...
<div class="sm-box">
    <div class="sm-header"><span class="sm-title">Ayarlar</span><button class="sm-close" onclick="document.getElementById('settings-modal').remove()"><i class="fas fa-xmark"></i></button></div>
    <div class="sm-tabs">
    ...
    <div class="sm-body">
        <div class="sm-section active" id="smp">
            <div style="text-align:center;margin-bottom:20px;">
                <img id="sm-avatar-preview" src="/avatars/${avatarStyle}.svg" style="width:80px;height:80px;border-radius:20px;border:2px solid rgba(0,242,254,0.3);background:#18181d;padding:4px;transition:all 0.3s;" alt="Avatar">
                ...
                <div style="display:flex;flex-wrap:wrap;justify-content:center;gap:8px;margin-top:10px;">
                    ${DICEBEAR_STYLES.map(s => `<button onclick="smSelectAvatar('${s.id}')" data-avatar-style="${s.id}" style="display:flex;flex-direction:column;align-items:center;gap:4px;padding:6px 8px;border-radius:10px;cursor:pointer;background:${s.id === avatarStyle ? 'rgba(0,242,254,0.12)' : 'rgba(255,255,255,0.04)'};border:1px solid ${s.id === avatarStyle ? 'rgba(0,242,254,0.4)' : 'rgba(255,255,255,0.08)'};transition:all 0.15s;min-width:64px;"><img src="/avatars/${s.id}.svg" style="width:36px;height:36px;border-radius:8px;background:#18181d;padding:3px;" alt="${s.label}"><span style="font-size:10px;color:var(--text-secondary);">${s.label}</span></button>`).join('')}
                </div>
            </div>
            <div class="sm-field"><label class="sm-lbl">Ad Soyad</label><input class="sm-inp" id="sm-name" value="${user.name || localStorage.getItem('user_name') || ''}" placeholder="Adınızı girin"></div>
            <div class="sm-field"><label class="sm-lbl">E-posta</label><input class="sm-inp" id="sm-email" value="${user.email || ''}" readonly style="opacity:0.5;cursor:not-allowed;" title="E-posta adresi değiştirilemez" onfocus="this.blur()"></div>
            <div style="display:flex;gap:8px;"><button class="sm-btn sm-primary" onclick="smSaveProfile()">Kaydet</button></div>
        ...
        <div class="sm-section" id="sma">
            <div class="sm-plan ${isPremium?'premium':'free'}">
                <div style="display:flex;align-items:center;justify-content:space-between;">
                    <div><div style="font-size:14px;font-weight:700;color:#fff;">${isPremium?'👑 Premium':'🆓 Ücretsiz Plan'}</div><div style="font-size:11px;margin-top:2px;color:${isCancelled?'#ffb400':'#8e8ea0'};">${isPremium?(isCancelled?`⚠ İptal edildi · ${periodEnd} tarihinde sona erecek`:(periodEnd?`Yenileme: ${periodEnd}`:'Aktif abonelik')):'Günde 50 mesaj hakkı'}</div></div>
            ...
            <div style="display:flex;justify-content:space-between;font-size:11px;color:#8e8ea0;"><span>${todayUsed} mesaj gönderildi</span><span>${isPremium?'Sınırsız':dailyLimit+' limit'}</span></div>
            ...
            ${history.length===0?`<div style="font-size:13px;color:#55556a;padding:10px 0;">Henüz ödeme geçmişi yok.</div>`:history.slice(0,6).map(h=>`<div class="sm-pay"><div><div style="font-size:13px;color:#eeeef0;font-weight:500;">Premium Abonelik</div><div style="font-size:11px;color:#55556a;margin-top:2px;">${h.paid_at||h.date}</div></div><div style="font-size:14px;font-weight:700;color:#00e676;">₺${parseFloat(h.amount||0).toFixed(2)}</div></div>`).join('')}
            ${isPremium?`<div class="sm-div"></div><button class="sm-btn sm-ghost" style="width:100%;display:flex;justify-content:center;" onclick="smCancelSub()">Aboneliği İptal Et</button>`:''}
        </div>
        <div class="sm-section" id="smn">
            <div class="sm-row"><div><div class="sm-row-lbl">Platform Güncellemeleri</div><div class="sm-row-sub">Yeni özellik ve duyurular</div></div><label class="sm-toggle"><input type="checkbox" id="n-upd" checked onchange="smSaveNotifs()"><span class="sm-trk"></span></label></div>
            <div class="sm-div"></div>
            <div class="sm-row"><div><div class="sm-row-lbl">Güvenlik Bildirimleri</div><div class="sm-row-sub">Giriş ve hesap aktivitesi</div></div><label class="sm-toggle"><input type="checkbox" id="n-sec" checked onchange="smSaveNotifs()"><span class="sm-trk"></span></label></div>
            <div class="sm-div"></div>
            <div class="sm-row"><div><div class="sm-row-lbl">Fatura E-postaları</div><div class="sm-row-sub">Ödeme ve abonelik bildirimleri</div></div><label class="sm-toggle"><input type="checkbox" id="n-bil" checked onchange="smSaveNotifs()"><span class="sm-trk"></span></label></div>
            <div id="sm-nmsg" style="font-size:12px;color:#00e676;min-height:16px;margin-top:4px;"></div>
        </div>
        <div class="sm-section" id="smg">
            <div style="background:rgba(0,242,254,.04);border:1px solid rgba(0,242,254,.1);border-radius:10px;padding:13px;margin-bottom:18px;"><div style="font-size:12px;color:#00f2fe;font-weight:600;margin-bottom:4px;"><i class="fas fa-shield-halved"></i> OTP ile güvenli giriş aktif</div><div style="font-size:12px;color:#8e8ea0;line-height:1.6;">Hesabınız e-posta doğrulaması ile korunuyor.</div></div>
...
function showSupportModal() {
    ...
    if (modal) {
        modal.style.display = 'flex'; _supportType = 'general';
        document.querySelectorAll('.support-type-btn').forEach(b => { b.style.background = 'rgba(255,255,255,0.04)'; b.style.borderColor = 'rgba(255,255,255,0.08)'; });
        const msgEl = document.getElementById('support-message'); if (msgEl) msgEl.value = '';
        const res = document.getElementById('support-result'); if (res) res.style.display = 'none';
    }
}
...
async function submitSupport() {
    ...
    if (!msg || msg.length < 5) { showToast('Lütfen mesajınızı yazın', 'warn'); return; }
    const token = localStorage.getItem('token');
    try {
        await fetch(PROXY_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'feedback', token, conversation_id: currentConversationId || '', user_query: `[DESTEK - ${_supportType.toUpperCase()}]`, assistant_response: msg, rating: _supportType === 'bug' ? -1 : 1, comment: `[${_supportType}] ${msg}` }) });
        const res = document.getElementById('support-result'); if (res) res.style.display = 'block';
        showToast('Mesajınız alındı, teşekkürler!', 'success');
        setTimeout(() => { document.getElementById('support-modal').style.display = 'none'; }, 2000);
    } catch(e) { showToast('Gönderilemedi, tekrar deneyin', 'error'); }
}

function switchSidebarTab(tab) {
    const navChats = document.getElementById('nav-chats'), navApps = document.getElementById('nav-apps');
    if (tab === 'apps') {
        openAppsPage(); navApps?.classList.add('active'); navChats?.classList.remove('active');
        setTimeout(() => { navApps?.classList.remove('active'); navChats?.classList.add('active'); }, 300);
    }
}
...
function openSosyal() {
    ...
    if (!isPremium) { closeAppsPage(); showUpgradeModal('Sosyal Asistan'); return; }
    window.location.href = '/sosyal/';
}

function openAppsPage() {
    const page = document.getElementById('apps-fullpage');
    if (!page) return;
    window._appsPagePrevState = { centerStage: document.getElementById('center-stage')?.style.display || '', chatDisplay: document.getElementById('chat-display')?.style.display || '', mainContainer: document.getElementById('main-container')?.style.display || '' };
    page.style.display = 'block'; page.style.opacity = '0'; page.style.transform = 'translateY(12px)'; page.style.transition = 'opacity 0.22s ease, transform 0.22s ease';
    requestAnimationFrame(() => requestAnimationFrame(() => { page.style.opacity = '1'; page.style.transform = 'translateY(0)'; }));
    document.body.style.overflow = 'hidden'; page.scrollTop = 0;
}
...
function closeAppsPage() {
    ...
    setTimeout(() => {
        page.style.display = 'none'; page.style.opacity = ''; page.style.transform = ''; page.style.transition = '';
        document.body.style.overflow = '';
        const prev = window._appsPagePrevState || {};
        const centerStage = document.getElementById('center-stage'), chatDisplay = document.getElementById('chat-display'), mainContainer = document.getElementById('main-container');
...
function toggleConvDropdown() {
    const toggle = document.getElementById('conv-dropdown-toggle'), body = document.getElementById('conv-dropdown-body');
    if (!toggle || !body) return;
    const isOpen = body.classList.contains('open');
    if (isOpen) { body.classList.remove('open'); toggle.classList.remove('open'); }
...
async function smSaveProfile() {
    const token = localStorage.getItem('token'), name = document.getElementById('sm-name')?.value.trim(), msg = document.getElementById('sm-pmsg');
    try {
        const pendingStyle = localStorage.getItem('avatar_style_pending') || localStorage.getItem('avatar_style') || 'av-01';
        const res  = await fetch(PROXY_URL, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'update_profile', token, name, avatar_style: pendingStyle }) });
        const data = await res.json();
        if (data.status === 'success') {
            ...
            const savedStyle = localStorage.getItem('avatar_style_pending') || localStorage.getItem('avatar_style') || 'av-01';
            localStorage.setItem('avatar_style', savedStyle); localStorage.removeItem('avatar_style_pending');
            const userAv = document.getElementById('user-avatar'); if (userAv) userAv.src = `/avatars/${savedStyle}.svg`;
        } else { msg.style.color = '#ff4b4b'; msg.textContent = data.message || data.detail || 'Hata oluştu'; }
...
async function smSaveNotifs() {
    const token = localStorage.getItem('token');
    try {
        await fetch(PROXY_URL, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'update_notifications', token, email_updates: document.getElementById('n-upd')?.checked, email_security: document.getElementById('n-sec')?.checked, email_billing: document.getElementById('n-bil')?.checked }) });
        const msg = document.getElementById('sm-nmsg'); if(msg) { msg.textContent = '✓ Kaydedildi'; setTimeout(()=>msg.textContent='', 2000); }
    } catch(e) {}
}
...
async function smCancelSub() {
    ...
    try {
        ...
        const data = await res.json();
        showToast(data.message || 'Abonelik iptal edildi', 'info'); document.getElementById('settings-modal')?.remove(); await loadSubscriptionStatus();
    } catch(e) { showToast('Hata oluştu', 'error'); }
}
...
// ═══════════════════════════════════════════════════════════════
// ADAPTIVE THINKING — Tek Toggle (aç/kapa)
// ═══════════════════════════════════════════════════════════════

window._thinkingMode = localStorage.getItem('thinking_mode') || 'none';

function setThinkingMode(mode) {
    window._thinkingMode = mode;
    localStorage.setItem('thinking_mode', mode);
    const btn = document.getElementById('adaptive-toggle');
    if (btn) {
...
function toggleAdaptive() {
    const next = (window._thinkingMode === 'auto') ? 'none' : 'auto';
    setThinkingMode(next);
}

// Reasoning baloncuğu — bir mesaj için bir kez açılır
let _thinkBox = null;
let _thinkContent = null;
let _thinkStartTs = 0;

function _thinkOpen(botMsg) {
    if (_thinkBox) return;
    _thinkStartTs = Date.now();
    _thinkBox = document.createElement('div');
    _thinkBox.className = 'reasoning-box';
    _thinkBox.innerHTML = `
        <div class="reasoning-head" onclick="this.parentElement.classList.toggle('collapsed')">
            <span class="reasoning-orb">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 600">
                    <defs>
                        <radialGradient id="rb-orbMain" cx="42%" cy="38%" r="62%">
                            <stop offset="0%" stop-color="#a0b8d8"/>
                            <stop offset="25%" stop-color="#5ab8d4"/>
                            <stop offset="60%" stop-color="#00c8d8"/>
                            <stop offset="100%" stop-color="#009aaa"/>
                        </radialGradient>
                        <radialGradient id="rb-highlight" cx="35%" cy="30%" r="45%">
                            <stop offset="0%" stop-color="#c0a8e0" stop-opacity="0.65"/>
                            <stop offset="60%" stop-color="#8898c8" stop-opacity="0.25"/>
                            <stop offset="100%" stop-color="#4488bb" stop-opacity="0"/>
                        </radialGradient>
                        <radialGradient id="rb-rimLight" cx="50%" cy="85%" r="40%">
                            <stop offset="0%" stop-color="#00e8f8" stop-opacity="0.4"/>
                            <stop offset="100%" stop-color="#00c8d8" stop-opacity="0"/>
                        </radialGradient>
                    </defs>
                    <circle cx="300" cy="300" r="213" fill="none" stroke="#00d8e8" stroke-width="1.5" opacity="0.25"/>
                    <circle cx="300" cy="300" r="210" fill="url(#rb-orbMain)"/>
                    <circle cx="300" cy="300" r="210" fill="url(#rb-highlight)"/>
                    <circle cx="300" cy="300" r="210" fill="url(#rb-rimLight)"/>
                    <ellipse cx="265" cy="205" rx="95" ry="52" fill="white" opacity="0.10" transform="rotate(-15,265,205)"/>
                </svg>
            </span>
            <span class="reasoning-title">Düşünüyor...</span>
            <span class="reasoning-time"></span>
            <span class="reasoning-toggle">▼</span>
        </div>
        <div class="reasoning-content"></div>
    `;
    _thinkContent = _thinkBox.querySelector('.reasoning-content');
    const target = botMsg?.content || botMsg;
    if (target && target.appendChild) {
        const ti = target.querySelector('.typing-indicator');
        if (ti) ti.remove();
        target.insertBefore(_thinkBox, target.firstChild);
    }
}

function _thinkAppend(text) {
    if (!_thinkContent || !text) return;
    _thinkContent.textContent += text;
    _thinkContent.scrollTop = _thinkContent.scrollHeight;
}

function _thinkClose() {
    if (!_thinkBox) return;
    const elapsed = Math.round((Date.now() - _thinkStartTs) / 1000);
    const title = _thinkBox.querySelector('.reasoning-title');
    const time = _thinkBox.querySelector('.reasoning-time');
    if (title) title.textContent = 'Düşündüm';
    if (time) time.textContent = `(${elapsed}sn)`;
    setTimeout(() => { if (_thinkBox) _thinkBox.classList.add('collapsed'); }, 2000);
    _thinkBox = null;
    _thinkContent = null;
}

function _thinkResetState() {
    _thinkBox = null;
    _thinkContent = null;
    window._thinkActive = false;
    window._thinkBuf = '';
}

// Sayfa yenilenince eski reasoning baloncuklarını temizle
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.reasoning-box').forEach(el => el.remove());
    _thinkResetState();
});

// Toggle butonunu input alanının soluna koy
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
        if (document.getElementById('adaptive-toggle')) return;
        const inputArea = document.querySelector('.input-wrapper') || document.querySelector('.input-area');
        if (!inputArea) return;

        const btn = document.createElement('button');
        btn.id = 'adaptive-toggle';
        btn.type = 'button';
        btn.className = 'adaptive-toggle';
        btn.title = 'Adaptive Thinking — kapalıyken anında cevap, açıkken zor sorularda düşünür';
        btn.innerHTML = `
            <span class="at-icon">🧠</span>
            <span class="at-label">Adaptive</span>
            <span class="at-dot"></span>
        `;
        btn.onclick = toggleAdaptive;

        inputArea.insertBefore(btn, inputArea.firstChild);
        setThinkingMode(window._thinkingMode);
    }, 500);
});